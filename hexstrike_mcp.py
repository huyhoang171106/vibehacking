...
""",
    ...
    # ============================================================================
    # CTF SOLVER TOOLS - Modular Challenge Automation
    # ============================================================================

    @mcp.tool()
    def ctf_classify_challenge(description: str, files: str = "", hints: str = "", url: str = "") -> Dict[str, Any]:
        """
        Automatically classify CTF challenge category and recommend tools.

        Detects challenge type from description, files, and hints using pattern matching
        and keyword analysis. Returns category (CRYPTO/PWN/WEB/REVERSE/FORENSICS/etc.),
        confidence score, subtype, and recommended tools for solving.

        Args:
            description: Challenge description text
            files: Comma-separated list of challenge files (optional)
            hints: Comma-separated hints or clues (optional)
            url: Challenge URL if web-based (optional)

        Returns:
            Classification results with category, confidence, recommended tools
        """
        file_list = [f.strip() for f in files.split(",")] if files else []
        hint_list = [h.strip() for h in hints.split(",")] if hints else []
        ...
        logger.info(f"{HexStrikeColors.FIRE_RED}🎯 Classifying CTF challenge{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/classify", data)

        if result.get("success"):
            category = result.get("category", "unknown")
            confidence = result.get("confidence", 0)
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Classified as: {category} (confidence: {confidence:.2%}){HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def ctf_rsa_auto_attack(n: str, e: str = "65537", c: str = "", timeout: int = 60) -> Dict[str, Any]:
        """
        Automatic RSA attack selection and execution.

        Analyzes RSA parameters and automatically selects the best attack method:
        - Wiener's attack (large e, small d)
        - Fermat factorization (close primes)
        - FactorDB lookup (known factors)
        - Small exponent attack (e=3)
        - Pollard's rho/p-1

        Args:
            n: RSA modulus (decimal or hex string)
            e: Public exponent (default: 65537)
            c: Ciphertext to decrypt (optional, decimal or hex)
            timeout: Maximum execution time in seconds

        Returns:
            Attack results with p, q, d, plaintext, and flag if recovered
        """
        data = {"n": n, "e": e, "c": c, "timeout": timeout}

        logger.info(f"{HexStrikeColors.FIRE_RED}🔐 Launching RSA auto-attack (n={n[:20]}...){HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/rsa/auto-attack", data)

        if result.get("success"):
            attack_type = result.get("attack_type", "unknown")
            logger.info(f"{HexStrikeColors.SUCCESS}✅ RSA cracked using {attack_type} attack{HexStrikeColors.RESET}")
            if result.get("flag"):
                logger.info(f"{HexStrikeColors.HIGHLIGHT_GREEN}🚩 FLAG: {result.get('flag')}{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def ctf_rsa_wiener_attack(n: str, e: str, c: str = "") -> Dict[str, Any]:
        """
        Wiener's attack for RSA with large public exponent and small private exponent.

        Effective when d < N^0.25 and e is very large. Uses continued fraction
        expansion to recover the private key. Common in CTF challenges with
        intentionally weak key generation.

        Args:
            n: RSA modulus (decimal or hex string)
            e: Public exponent (must be large)
            c: Ciphertext to decrypt (optional)

        Returns:
            Attack results with recovered d and plaintext if successful
        """
        data = {"n": n, "e": e, "c": c}

        logger.info(f"{HexStrikeColors.FIRE_RED}🔓 Attempting Wiener's attack{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/rsa/wiener", data)

        if result.get("success"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Wiener's attack successful{HexStrikeColors.RESET}")
            if result.get("flag"):
                logger.info(f"{HexStrikeColors.HIGHLIGHT_GREEN}🚩 FLAG: {result.get('flag')}{HexStrikeColors.RESET}")
    ...
    @mcp.tool()
    def ctf_rsa_fermat_attack(n: str, e: str = "65537", c: str = "", max_iterations: int = 10000) -> Dict[str, Any]:
        """
        Fermat factorization for RSA with close prime factors.

        Works when p and q are very close to each other (|p-q| is small).
        Fast factorization using Fermat's method: finds x, y where n = x² - y².
        Common in CTF challenges with weak key generation.

        Args:
            n: RSA modulus (decimal or hex string)
            e: Public exponent (default: 65537)
            c: Ciphertext to decrypt (optional)
            max_iterations: Maximum factorization attempts

        Returns:
            Factorization results with p, q, d, and plaintext if successful
        """
        data = {"n": n, "e": e, "c": c, "max_iterations": max_iterations}

        logger.info(f"{HexStrikeColors.FIRE_RED}🔢 Attempting Fermat factorization{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/rsa/fermat", data)

        if result.get("success"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Fermat factorization successful{HexStrikeColors.RESET}")
            if result.get("flag"):
                logger.info(f"{HexStrikeColors.HIGHLIGHT_GREEN}🚩 FLAG: {result.get('flag')}{HexStrikeColors.RESET}")
    ...
    @mcp.tool()
    def ctf_format_string_arbitrary_write(address: str, value: str, offset: int, arch: str = "x64") -> Dict[str, Any]:
        """
        Generate format string payload for arbitrary memory write.

        Creates a format string exploit payload that writes a specific value to a
        target memory address using %n/%hn/%hhn format specifiers. Handles proper
        padding and width calculations for reliable exploitation.

        Args:
            address: Target memory address (hex string, e.g., "0x601020")
            value: Value to write (hex string, e.g., "0xdeadbeef")
            offset: Format string offset (where input appears on stack)
            arch: Target architecture (x86/x64/arm/arm64, default: x64)

        Returns:
            Payload in hex and ASCII format with write description
        """
        data = {"address": address, "value": value, "offset": offset, "arch": arch}

        logger.info(f"{HexStrikeColors.FIRE_RED}📝 Generating format string write payload{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/format-string/arbitrary-write", data)

        if result.get("success"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Payload generated: {result.get('payload_hex')[:40]}...{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def ctf_format_string_got_overwrite(got_entry: str, target_addr: str, offset: int, arch: str = "x64") -> Dict[str, Any]:
        """
        Generate GOT overwrite payload for format string exploitation.

        Creates payload to overwrite a GOT (Global Offset Table) entry with a target
        address, redirecting program execution. Common technique for gaining code
        execution through format string vulnerabilities.

        Args:
            got_entry: GOT entry address to overwrite (hex, e.g., "0x601018")
            target_addr: Address to write into GOT (hex, e.g., "0x400686")
            offset: Format string offset (where input appears on stack)
            arch: Target architecture (x86/x64, default: x64)

        Returns:
            GOT overwrite payload with hex and ASCII representations
        """
        data = {"got_entry": got_entry, "target_addr": target_addr, "offset": offset, "arch": arch}

        logger.info(f"{HexStrikeColors.FIRE_RED}🎯 Generating GOT overwrite payload{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/format-string/got-overwrite", data)

        if result.get("success"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ GOT overwrite payload generated{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def ctf_rop_build_execve_chain(gadgets_json: str, binsh_addr: str, arch: str = "x64") -> Dict[str, Any]:
        """
        Build ROP chain for execve('/bin/sh') system call.

        Constructs a Return-Oriented Programming chain to execute /bin/sh using
        available gadgets. Handles register setup for syscall/int 0x80 on x64/x86.

        Args:
            gadgets_json: JSON string with gadgets dict: {"POP_RDI": {"address": "0x400686", "instructions": "pop rdi; ret"}, ...}
            binsh_addr: Address of "/bin/sh" string in memory (hex)
            arch: Target architecture (x86 or x64, default: x64)

        Returns:
            Complete ROP chain in hex format with gadget details and chain dump
        """
        import json

        gadgets = json.loads(gadgets_json)
        data = {"gadgets": gadgets, "binsh_addr": binsh_addr, "arch": arch}

        logger.info(f"{HexStrikeColors.FIRE_RED}⛓️  Building ROP execve chain{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/rop/build-execve", data)

        if result.get("success"):
            chain_len = result.get("chain_length", 0)
            logger.info(f"{HexStrikeColors.SUCCESS}✅ ROP chain built: {chain_len} bytes{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def ctf_rop_find_gadgets(binary_path: str, arch: str = "x64") -> Dict[str, Any]:
        """
        Extract ROP gadgets from a binary executable.

        Analyzes binary to find useful ROP gadgets (pop/mov/xor/syscall/ret sequences).
        Returns gadgets with addresses, instructions, types, and affected registers.

        Args:
            binary_path: Absolute path to binary file to analyze
            arch: Target architecture (x86 or x64, default: x64)

        Returns:
            List of gadgets with addresses, instructions, types, and registers
        """
        data = {"binary_path": binary_path, "arch": arch}

        logger.info(f"{HexStrikeColors.FIRE_RED}🔍 Finding ROP gadgets in binary{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/rop/find-gadgets", data)

        if result.get("success"):
            count = result.get("gadget_count", 0)
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Found {count} ROP gadgets{HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def ctf_web_ssrf_discover(target: str, endpoints: str = "", parameters: str = "", timeout: int = 30) -> Dict[str, Any]:
        """
        Discover Server-Side Request Forgery (SSRF) vulnerability chains.

        Tests target endpoints for SSRF by probing various protocols (http/https/file/
        gopher/dict/ftp), cloud metadata access, and internal network access. Returns
        exploitable chains with evidence.

        Args:
            target: Base target URL (e.g., "http://example.com")
            endpoints: Comma-separated list of endpoints to test (e.g., "/api/fetch,/proxy")
            parameters: Comma-separated URL parameters to test (e.g., "url,link,fetch")
            timeout: Request timeout in seconds

        Returns:
            SSRF chains found with protocols, internal access, and exploitation steps
        """
        endpoint_list = [e.strip() for e in endpoints.split(",")] if endpoints else []
        param_list = [p.strip() for p in parameters.split(",")] if parameters else []

        data = {"target": target, "endpoints": endpoint_list, "parameters": param_list, "timeout": timeout}

        logger.info(f"{HexStrikeColors.FIRE_RED}🌐 Discovering SSRF chains{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/web/ssrf-discover", data)

        if result.get("success"):
            chains = result.get("chains_found", 0)
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Found {chains} SSRF chain(s){HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def ctf_web_race_condition(target: str, endpoint: str, payload: str, concurrent: int = 100) -> Dict[str, Any]:
        """
        Exploit race condition vulnerabilities with concurrent requests.

        Sends many concurrent requests to exploit timing-based race conditions.
        Useful for bypassing checks, duplicate transactions, or accessing restricted
        resources. Analyzes response anomalies and timing.

        Args:
            target: Base target URL
            endpoint: Endpoint path to target (e.g., "/api/transfer")
            payload: JSON string with request payload
            concurrent: Number of concurrent requests (default: 100)

        Returns:
            Race condition results with success rate, anomalies, and potential flag
        """
        import json

        payload_dict = json.loads(payload)
        data = {"target": target, "endpoint": endpoint, "payload": payload_dict, "concurrent": concurrent}

        logger.info(f"{HexStrikeColors.FIRE_RED}⚡ Exploiting race condition ({concurrent} requests){HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/web/race-condition", data)

        if result.get("success"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Race condition exploited successfully{HexStrikeColors.RESET}")
            if result.get("flag"):
                logger.info(f"{HexStrikeColors.HIGHLIGHT_GREEN}🚩 FLAG: {result.get('flag')}{HexStrikeColors.RESET}")
    ...
    @mcp.tool()
    def ctf_web_prototype_pollution(target: str, endpoints: str = "") -> Dict[str, Any]:
        """
        Scan for JavaScript prototype pollution vulnerabilities.

        Tests endpoints for prototype pollution by injecting __proto__ and constructor
        payloads. Detects if Object.prototype can be polluted to gain privileges or
        bypass security checks.

        Args:
            target: Base target URL
            endpoints: Comma-separated endpoints to test (e.g., "/api/update,/settings")

        Returns:
            Scan results with vulnerable endpoints, payloads, and evidence
        """
        endpoint_list = [e.strip() for e in endpoints.split(",")] if endpoints else []
        data = {"target": target, "endpoints": endpoint_list}

        logger.info(f"{HexStrikeColors.FIRE_RED}🔬 Scanning for prototype pollution{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/web/prototype-pollution", data)

        if result.get("success"):
            vulns = len(result.get("vulnerabilities", []))
            logger.info(f"{HexStrikeColors.SUCCESS}✅ Found {vulns} prototype pollution vulnerability(ies){HexStrikeColors.RESET}")

        return result

    @mcp.tool()
    def ctf_web_jwt_attack(token: str, target: str = "", endpoint: str = "") -> Dict[str, Any]:
        """
        JWT token analysis and attack automation.

        Analyzes JWT structure, tests common vulnerabilities (algorithm confusion,
        weak secrets, none algorithm, key confusion), and attempts token forgery.
        Can test forged tokens against target endpoint.

        Args:
            token: JWT token to analyze and attack
            target: Base target URL for testing forged tokens (optional)
            endpoint: API endpoint for token validation (optional)

        Returns:
            JWT analysis with vulnerabilities, forged tokens, and attack results
        """
        data = {"token": token, "target": target, "endpoint": endpoint}

        logger.info(f"{HexStrikeColors.FIRE_RED}🎫 Analyzing JWT token{HexStrikeColors.RESET}")
        result = hexstrike_client.safe_post("api/ctf/solvers/web/jwt-attack", data)

        if result.get("success"):
            logger.info(f"{HexStrikeColors.SUCCESS}✅ JWT analysis complete{HexStrikeColors.RESET}")
